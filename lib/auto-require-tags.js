...
module.exports = {
	config: {
		prefix: {
			type: 'string',
			default: 'ui/',
			title: 'prefix for every tag',
			description: ''
		},
		filters: {
			type: 'string',
			default: '',
			title: 'tags not to add contains these words',
			description: 'such as "x-page- y-page-", use whitespace as separator'
		}
	},
	subscriptions: null,
		...
		while( ( tag = HTML_TAGS.exec( editorContent ) ) !== null ) {
			...
			if( tagName.indexOf( '-' ) > 0 ) {
				...
				for( var i = 0, len = filters.length; i < len; i++ ){
					if( tagName.indexOf( filters[ i ] ) >= 0 ){
						isValid = false;
					}
				}
